...
async function importLutinRouge() {
  try {
    console.log('📖 Lecture du fichier recettes-lutinRouge.json...');
    const recipesPath = join(__dirname, '..', 'recettes-lutinRouge.json');
    const recipesData = JSON.parse(readFileSync(recipesPath, 'utf8'));
    console.log(`✅ ${recipesData.length} recettes trouvées\n`);

    console.log('📝 Import (upsert) des recettes Lutin Rouge dans la collection `recipes`...');
    let count = 0;
    for (const recipe of recipesData) {
      const recipeDoc = {
        ...recipe,
        // Marquer ces recettes comme "Lutin rouge" pour les distinguer dans le site
        tag: 'Lutin rouge',
